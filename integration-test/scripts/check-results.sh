...
if [[ "$CIRCLECI" ]]; then
    ...
    if [[ "$INTEGRATIONTEST_CLOUDPROVIDER" == "MOCK" ]]; then
      docker run --rm -it \
       ...
       -v $(PWD)/integcb/docker_stats/pg_stat_network_io.out:/tmp/pg_stat.out \
       --env INTEGRATION_TEST_MAX_POSTGRES_OUTPUT=${max_pg_network_output} \
       openjdk:11-jdk \
       jshell /opt/analyse_postgres_stat.jsh |  grep "POSTGRES" > ./integcb/docker_stats/pg_stat_network_io_analysed.out;

      pg_res=$(cat ./integcb/docker_stats/pg_stat_network_io_analysed.out);
